...
def evaluate_model(model, X_test, y_test, epoch, device):
    ...
    with torch.no_grad():  # 不计算梯度
        ...
        # 计算准确率
        accuracy = (predicted.cpu().numpy() == y_test_indices).mean()  # 将预测移动回CPU并计算准确率
...
class CNNModel(nn.Module):
    ...
    def forward(self, x):
        # x = x.unsqueeze(1)  # 增加通道维度，形状为 (batch_size, 1, 50, 300)
        # 进行卷积操作
...
X_train, X_test, y_train, y_test = train_test_split(sentences_array, tags_array,
                                                    test_size=0.2, random_state=42, stratify=tags_array)
...
for epoch in range(num_epochs):
    ...
    # 转为Tensor并移动到GPU
    inputs = torch.tensor(X_train_tensor, dtype=torch.float32).to(device)  # 输入 shape: (batch_size, 50, 300)
    labels = torch.tensor(y_train, dtype=torch.float32).to(device)  # 标签 shape: (batch_size, 2)

    outputs = model(inputs)  # outputs shape: (batch_size, 2)

    loss = criterion(outputs, labels)  # 如果标签是0和1，确保labels是float类型
