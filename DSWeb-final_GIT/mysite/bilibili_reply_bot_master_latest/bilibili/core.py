...
# import bilibili_reply_bot_master_latest.config.base_config as config
# import bilibili_reply_bot_master_latest.config as config
import random
...
from bilibili_reply_bot_master_latest.base.base_crawler import AbstractCrawler
from bilibili_reply_bot_master_latest.store import lite
from bilibili_reply_bot_master_latest.tools.utils import utils
...
class BilibiliCrawler(AbstractCrawler):
    ...
    async def start(self):
        ...
        async with async_playwright() as playwright:
            ...
            await self.browser_context.add_init_script(path="bilibili_reply_bot_master_latest/libs/stealth.min.js")#更改相对路径
    ...
    async def get_comments(self, video_id: str, semaphore: asyncio.Semaphore):
        ...
        async with semaphore:
            try:
                from bilibili_reply_bot_master_latest.config import base_config as config
                ...
                # 使用 traceback 捕获完整的堆栈信息
                error_message = f"[BilibiliCrawler.get_comments] 发生未捕获的异常: {e}\n{traceback.format_exc()}"
                print(error_message)  # 输出到控制台
                utils.logger.error(error_message)  # 写入日志
