...
from bilibili_reply_bot_master_latest.store import lite
import bilibili_reply_bot_master_latest.videosearch as videosearch
from bilibili_reply_bot_master_latest.bilibili import BilibiliCrawler
from bilibili_reply_bot_master_latest.reply import get_reply_by_oid
import time
import csv
import bilibili_reply_bot_master_latest.config

from playwright.async_api import (BrowserContext, BrowserType, Page,
                                  async_playwright)

from bilibili_reply_bot_master_latest.bilibili.login import BilibiliLogin
from bilibili_reply_bot_master_latest.bilibili.client import BilibiliClient
from bilibili_reply_bot_master_latest.tools import utils
...
async def Crawer_new(keyword,DAY_BEFORE=0):

    bilibili_reply_bot_master_latest.config.DAY_BEFORE = DAY_BEFORE
    bili_crawler = BilibiliCrawler()
    # 登录
    await bili_crawler.start()

    # keyword=input("请输入视频关键字：")
    search_result = videosearch.search_bilibili_videos(keyword, bilibili_reply_bot_master_latest.config.CRAWLER_MAX_SEARCH_PAGE_COUNT)
